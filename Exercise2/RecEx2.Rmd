...
- Recognition of handwritten digits. That'd be prediction
...
- With regression we're interested in developing a statistical model relating different random variables, say the number of cups of coffee each day to hours of sleep each day. With such a regression model we could say something about the relationship, which would be inference. 
...
- If the model is not too rigid, another flexible model will usually yield higher test error as the data can be overfitted more easily by flexible models. 

b) Does a small variance imply that the data has been under- or overfit?
- Small model variance implies (to a degree) underfitting, as high model variance corresponds to overfitting the data.

c) Relate the problem of over- and underfitting to the bias-variance trade-off.
- When overfitting we're introducing high variance to our model since a new training dataset will change the model considerably. When underfitting data we're introducing high bias to our model since we're getting errors due to the model being too simple. Variance is related to the flexibility of the model and bias is related to the rigidity of the model. The MSE varies with the variance and squared bias of the model, so decreasing both is the goal to minimize MSE. Initially, the bias decreases at a higher rate than the variance increases so the MSE decreases. At some point, the variance grows faster than the bias decreases. Thus there is a point inbetween where we find a minimum MSE wrt. both variance and bias. This is the bias-variance tradeoff.
...
- Auto has 392 different observations of 9 features. mpg, displacement, horsepower, weight and acceleration are quantitative. cylinders, year, origin and name are qualitative.
...
- Both HP and Weight increases with displacement, and horsepower increases with weight. Displacement, horsepower and weight seems to decrease with increasing mpg.
...
- From the boxplots of the qualitative predictors we see a correlation between both cylinders and origin. Also from the previous scatterplots we also see correlation of mpg with displacement, horsepower and weight.
...
for(i in 1:nord){
  ...
  for(j in 1:M){
    ...
    # and make a prediction for the values in x for each model, put it in the list of predictions
...
# indicating simulation number and polynomial degree (simulation number is new for each row, 
# polynomial degree is the same for each row but different for each matrix)
...
# Extract each matrix and bind them to one large matrix by row so sim-number and polynomial
# degree columns align downwards, new polynomial order simulation for each 100 rows
...
# Act on columns of stacked_matrices_df that are NOT simulation_num and poly_degree
# Reshape stacked matrix into a long long long list of values which have a simulation number
# and polynomial degree as well as a name indicating which x value it corresponds to,
# call this new long column "y"
...
- For degree 10 and 20 we see increasing variance in the model as its overfitting the data. For degree 1 we see that the model is too simple so the data is coarsely underfitted. The degree 2 model seems to fit the data best.
---
...
Study the final plot you produced: when the flexibility increases (poly increase), what happens with
    i) the squared bias
    - The squared bias drops off as flexibility increases
    ii) the variance
    - The variance increases with increasing flexibility
    iii) the irreducible error?  
    - The irreducible error stays at a constant value of 4
