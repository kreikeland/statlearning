...
<<<<<<< HEAD
- The p-value for weight is very small, so there is strong evidence that the weight of a car influences its mpg metric. Since $\beta_{\text{weight}}$ = `r reg_model$coefficients['weight']` the cars predicted mpg metric will be lower if its weight increases. If a cars weight increases by 1000kg, its mpg metric will _increase_ by `r reg_model$coefficients['weight'] * 1000`.
